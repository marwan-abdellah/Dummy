...
void iB_cuFFTShift_2D::FFTShift_2D_Float_CUDA_
(int size_X, int size_Y, Sheet* xlSheet, int nLoop, dim3 cuGrid, dim3 cuBlock)
{
	INFO( "2D FFT Shift Float, Array:" + ITS(size_X) + "x" + ITS(size_Y)	\
		+ " Block:" + ITS(cuBlock.x) + "x" + ITS(cuBlock.y) 				\
	...
	if (xlSheet)
	{
		for (int iLoop = 0; iLoop < nLoop; iLoop++)
		{
			...
			for (int i = 0; i < size_X; i++)
				for (int j = 0; j < size_Y; j++)
				{
					xlSheet->writeNum(/*ctr*/ 0 + 2, ((iLoop * 2 ) + 0), arr_2D_flat_float[ctr]);
					ctr++;
				}
			...
			for (int i = 0; i < size_X; i++)
				for (int j = 0; j < size_Y; j++)
				{
					xlSheet->writeNum(/*ctr*/ 0 + 2, ((iLoop * 2 ) + 1), arr_2D_flat_float[ctr]);
					ctr++;
				}
