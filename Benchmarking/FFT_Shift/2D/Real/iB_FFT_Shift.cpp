...
#include "iB_FFT_Shift.h"
#include <stdio.h>
#include <stdlib.h>

#define START_ROW_DATA 3

namespace iB_FFT_Shift
{
	Book* xlBook;
	Sheet* xlSheet;
	int ctr = 0;
...
void iB_FFT_Shift::FFTShift_2D_Float
(int size_X, int size_Y, Sheet* xlSheet, int nLoop, dim3 cuGrid, dim3 cuBlock)
{
	...
	if (xlSheet)
	{
		for (int iLoop = 0; iLoop < nLoop; iLoop++)
		{
			...
			arr_2D_float = MEM_ALLOC_2D (float, size_X, size_Y);
			arr_2D_flat_float = MEM_ALLOC_1D(float, size_X * size_Y);
...
void iB_FFT_Shift::FFTShift_2D_Double
(int size_X, int size_Y, Sheet* xlSheet, int nLoop, dim3 cuGrid, dim3 cuBlock)
{
	...
	if (xlSheet)
	{
		for (int iLoop = 0; iLoop < nLoop; iLoop++)
		{
			...
			// Allocation: 2D, Flat, Device
			arr_2D_double = MEM_ALLOC_2D(double, size_X, size_Y);
			arr_2D_flat_double = MEM_ALLOC_1D(double, size_X * size_Y);
